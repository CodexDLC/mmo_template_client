# res://autoload/network/HttpAuth.gd
extends Node

signal login_completed(ok: bool, data_or_err)
signal refresh_completed(success, data_or_error)
# Сигнал для оповещения о результате регистрации.
signal register_completed(ok: bool, data_or_err)
...
func register(credentials: Dictionary) -> void:
	...
	Log.info("Sending register request to " + url)
	_last_request_type = "register"
	_http_request.request(url, headers, HTTPClient.METHOD_POST, body)
...
func _update_session_and_schedule_refresh(data: Dictionary) -> void:
	...
	Session.set_tokens(access, refresh_tok, expires_in) # Здесь expires_in - это секунды
	_schedule_refresh()
